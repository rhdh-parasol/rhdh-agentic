...
name: rhdh-workspace
description: >-
  Navigate to the correct workspace in the rhdh-agentic monorepo, install
...
1. **Issue labels** — look for `workspace/<name>` labels (e.g., `workspace/backstage-agent`)
2. **Issue title or body** — look for workspace or plugin names (e.g., "backstage-agent", "agent-common")
3. **PR changed files** — if files are under `workspaces/<name>/`, that is the target
4. **Plugin package names** — map `@internal/plugin-<name>` to its workspace
...
After install, workspace-specific tools become available (e.g., `yarn tsc`).
